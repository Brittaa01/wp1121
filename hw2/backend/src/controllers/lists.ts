...
export const getLists = async (_: Request, res: Response<GetListsResponse>) => {
  try {
    ...
    const listsToReturn = await Promise.all(lists.map(async (list) => {
      const length = await SongModel.where({ list_id: list.id }).countDocuments().exec();
      return {
        id: list.id,
        name: list.name,
        length: length,
...
export const getList = async (
  ...
  try {
    ...
    //const length = await SongModel.find({list_id: id});
    const length = await SongModel.where({ list_id: id }).countDocuments();
